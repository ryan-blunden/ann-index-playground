...
class ActianBackend:
    ...
    def ensure_initial_artifacts(self, progress: ProgressCallback | None = None) -> None:
        total_steps = 2 + int(self.settings.include_hnsw)
        current_step = 0
    ...
    def ivf_summary(self, nlist: int, nprobe: int | None = None) -> str:
        return "IVF is not supported in this Actian release."
    ...
    def _upsert_batch_with_retry(self, collection_name: str, batch: list[PointStruct], start_index: int) -> None:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                with self._client() as client:
                    client.points.upsert(collection_name, batch)
                return
            except CollectionNotFoundError:
                raise
            except (ChannelClosedError, VectorAIConnectionError, RateLimitError, VectorAIError) as exc:
                if isinstance(exc, VectorAIError) and exc.code not in {429, 500, 503}:
                    raise
                if attempt == max_attempts:
                    raise
                self._wait_for_service()
        ...
        *,
        ...
        for repeat in range(repeats):
            ...
            for query_index, query in enumerate(queries):
                start = time.perf_counter()
                results = self._search_with_retry(client, collection_name, query, k, search_params)
    ...
    def _warmup_queries(
        ...
        for query in queries[: min(10, len(queries))]:
            self._search_with_retry(client, collection_name, query, k, search_params)
    ...
    def _search_with_retry(
        ...
        *,
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                return client.points.search(collection_name, vector=query.tolist(), limit=k, params=search_params)
            except (RateLimitError, ChannelClosedError, VectorAIConnectionError, VectorAIError) as exc:
                if isinstance(exc, VectorAIError) and exc.code not in {429, 500, 503}:
                    raise
                if attempt == max_attempts:
                    raise
                time.sleep(min(5.0, 0.5 * attempt))
        raise RuntimeError(f"Actian search failed unexpectedly for collection {collection_name!r}.")
